...
if [[ $1 == "run" ]]; then
		...
		if [[ $cc_type == "markovian" ]]; then
				...
				mm-delay $min_delay \
						...
						./run-genericcc-sender.sh "$bin_dir/sender serverip=$receiver_ip cctype=markovian offduration=0 delta_conf=do_ss:keep_ext_min_rtt:$delta_conf" $nsrc $traffic_type $run_time $on_duration $off_duration \
