...
if [[ $1 == "run" ]]; then
		...
		for tcp_dir in $output_directory/*::*; do
				...
				throughput=`grep throughput $tcp_dir/$tcp::$loss_rate.stats | awk -F ' ' '{print $3}'`
				delay=`grep 95.*queueing $tcp_dir/$tcp::$loss_rate.stats | awk -F ' ' '{print $6}'`
