...
run_time=20000 # in ms
on_duration=20000 # in ms
...
if [[ $1 == "run" ]]; then
		...
		if [[ $cc_type == "markovian" ]]; then
				...
				mm-delay 0 \
						...
						./run-genericcc-sender.sh "$bin_dir/sender serverip=$receiver_ip cctype=markovian offduration=0 delta_conf=do_ss:$delta_conf" $nsrc $traffic_type $run_time $on_duration $off_duration \
				...
				mm-delay $min_delay \
						mm-loss uplink $loss_rate \
						mm-link $trace_uplink $trace_downlink --uplink-log $of_name::$loss_rate.uplink --downlink-log $of_name::$loss_rate.downlink $queue_length_params \
						./run-genericcc-sender.sh "$bin_dir/sender serverip=$receiver_ip cctype=remy if=$rat_file" $nsrc $traffic_type $run_time $on_duration $off_duration \
						1> $of_name::$loss_rate.stdout 2> $of_name::$loss_rate.stderr
